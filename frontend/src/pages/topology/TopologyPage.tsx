...
export default function TopologyPage() {
  ...
  useEffect(() => {
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      // Limit zoom
      if (camera.position.length() * factor > 10 && camera.position.length() * factor < 1000) {
  ...
  useEffect(() => {
    ...
    const criticalCount = realVulns.filter((v: Vulnerability) => v.severity === 'critical').length
    const highCount = realVulns.filter((v: Vulnerability) => v.severity === 'high').length
    ...
    realVulns.forEach((vuln: Vulnerability) => {
      ...
      nodeVuln.description = vuln.name || (vuln as any).templateId || `${vuln.severity} 漏洞`
    ...
    const addNode = (id: string, name: string, type: string, data?: any) => {
      if (!nodeMap.has(id)) {
        ...
        if (!data?.color) {
          switch (type) {
            case 'domain': color = '#a78bfa'; val = 8; break
            case 'ip': color = '#60a5fa'; val = 6; break
            case 'web': color = '#34d399'; val = 6; break
            case 'app': color = '#f472b6'; val = 6; break
    ...
    simulationRef.current = d3.forceSimulation<GraphNode, GraphLink>(nodes)
      .force('link', d3.forceLink<GraphNode, GraphLink>(links).id((d: any) => d.id).distance(50))
      ...
      .on('tick', () => {
        ...
        links.forEach(link => {
          if (link.threeObject) {
            ...
            link.threeObject.geometry.setAttribute('position', new THREE.BufferAttribute(positions, 3))
    ...
    <div className="h-[calc(100vh-8rem)] w-full relative bg-black overflow-hidden rounded-lg border border-slate-800 shadow-2xl">
      ...
      {(isFetching || isLoadingGraph) && (
        <div className="absolute inset-0 flex flex-col items-center justify-center z-50 bg-black/80 backdrop-blur-sm">
          ...
          <p className="text-slate-400 animate-pulse">Initializing Galaxy Simulation...</p>
        </div>
      )}
          ...
          <Card className="bg-slate-950/90 border-slate-800 backdrop-blur-md text-slate-100 shadow-2xl overflow-hidden">
            ...
            <div className="relative p-6 pb-4 border-b border-slate-800/50">
              ...
              <div className="flex justify-between items-start mb-1">
                <span className="text-[10px] font-bold tracking-widest text-cyan-400 uppercase">Asset Details</span>
                <Button 
                  variant="ghost" 
                  size="icon" 
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-mono text-cyan-400">{selectedNode.id.toUpperCase()}</span>
              </div>
            </div>
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-slate-900/50 rounded-lg p-3 border border-slate-800">
                  ...
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${selectedNode.data.status === 'active' ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]' : 'bg-red-500'}`}></div>
                    <span className={`font-medium ${selectedNode.data.status === 'active' ? 'text-green-400' : 'text-red-400'}`}>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between items-center py-1 border-b border-slate-800/50">
                  ...
                  <span className="font-mono text-slate-300">{selectedNode.data.ip || '10.5.0.1'}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-800/50">
                  <span className="text-slate-500">区域</span>
                  <span className="text-slate-300">{selectedNode.data.region || '未知区域'}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-800/50">
                  <span className="text-slate-500">负责人</span>
                  <span className="text-slate-300">{selectedNode.data.owner || 'Admin'}</span>
                </div>
                <div className="flex justify-between items-center py-1 border-b border-slate-800/50">
              ...
              <div className="bg-cyan-950/20 border border-cyan-900/30 rounded-lg p-4">
                <p className="text-xs text-slate-300 leading-relaxed">
                  {selectedNode.data.description || '节点数据加载完毕。该节点作为核心路由交换节点，承载主要数据流量。'}
                </p>
              </div>

              {/* Vulnerability Stats (If any) */}
              {selectedNode.data.vulns && selectedNode.data.vulns.length > 0 && (
                <div className="bg-red-950/20 border border-red-900/30 rounded-lg p-3">
                  <div className="text-xs text-red-400 mb-2 font-bold uppercase tracking-wider">Security Risks</div>
                  <div className="flex gap-2">
                    {selectedNode.data.vulns.filter((v: any) => v.severity === 'critical').length > 0 && (
                      <Badge className="bg-red-600 hover:bg-red-700 text-white border-0">
                        CRITICAL: {selectedNode.data.vulns.filter((v: any) => v.severity === 'critical').length}
                      </Badge>
                    )}
                    {selectedNode.data.vulns.filter((v: any) => v.severity === 'high').length > 0 && (
                      <Badge className="bg-orange-600 hover:bg-orange-700 text-white border-0">
                        HIGH: {selectedNode.data.vulns.filter((v: any) => v.severity === 'high').length}
                      </Badge>
                    )}
                    {selectedNode.data.vulns.filter((v: any) => v.severity === 'medium').length > 0 && (
                      <Badge className="bg-yellow-600 hover:bg-yellow-700 text-white border-0">
                        MED: {selectedNode.data.vulns.filter((v: any) => v.severity === 'medium').length}
                      </Badge>
                    )}
                  </div>
                </div>
      ...
      <div className="absolute top-6 left-6 z-10">
        <div className="flex items-center gap-2 px-4 py-2 bg-slate-900/80 backdrop-blur-md border border-slate-700 rounded-full shadow-lg">
          ...
          <span className="text-sm font-bold tracking-wider text-slate-100">资产星图</span>
        </div>
      </div>
